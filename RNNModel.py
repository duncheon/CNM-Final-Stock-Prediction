...
def RNNmodel(symbol, interval):
    ...
    model.add(SimpleRNN(128, return_sequences=True, input_shape=(
        x_train_data.shape[1], 1), activation="relu"))
    ...
    model.save("./models/rnn_" + symbol.lower() + "_" + f"{interval}" + ".h5")
...
def prediction(symbol, interval, total):
    ...
    loaded_model = load_model(f'./models/rnn_{symbol}_{intervalKey}.h5')
...
# def prediction(symbol, interval model):
# RNNmodel('btcusdt')
